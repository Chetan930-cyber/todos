...
const TodoList = () => {
  ...
  {allTodos.map((todo) => (
    <li
      key={todo.id}
      className={`flex flex-wrap justify-between items-center gap-2 p-4 rounded-lg shadow-md transition duration-300 
        ${edit.isEdit && edit.todo.id === todo.id ? "bg-yellow-300" : "bg-white"}`}
    >
      <span className="text-lg font-bold text-gray-800 flex-1 min-w-[150px] break-words">
        {todo.text}
      </span>
